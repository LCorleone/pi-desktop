...
> **Fork additions** (`LCorleone/pi-desktop`): See section 11 below.
...
## 11) Fork Additions (`LCorleone/pi-desktop`)

Capabilities added on top of the upstream feature set:

| Capability | Desktop mapping |
|---|---|
| Auto session naming via `pi --print` | ✅ Session titles generated by pi CLI itself (no config parsing, works with any provider/model) |
| Real PTY terminal backend | ✅ `portable-pty` Rust bridge replaces xterm.js addons (proper resize/I/O, no chat-canvas leakage) |
| Modified files cards + diff viewer | ✅ File cards with `diff +N -M` stats; red/green unified diff in file panel from edit tool args |
| Per-turn stats footer | ✅ tps (tokens/sec) + cache hit-rate after each agent turn |
| Subagent display overhaul | ✅ Robot icon, `[Type] prompt` summary, structured completion card (parses `<task-notification>` XML) |
| Thinking display overhaul | ✅ Shimmer animation, auto-expand during streaming, auto-collapse when done, caret toggle |
| Tool-type icons + status dots | ✅ 6 categories (terminal/read/write/edit/search/agent) with label pills + colored status indicators |
| Workflow timeline status rail | ✅ Vertical rail with colored nodes (blue running, red failed, green done) |
| Sans-refresh typography | ✅ System sans-serif for chrome; code/terminal/tools stay JetBrains Mono via `--font-family-mono` |
| Streaming render coalescing | ✅ `requestAnimationFrame` batches text-delta renders to one per frame (~60fps), fixes long-session stutter |
| CI auto-release on merge | ✅ `release-on-merge.yml` auto-builds + tags + publishes when `releases/v*.md` hits main |
| Windows window controls | ✅ Minimize/maximize/close buttons on sidebar (native Windows-style) |
| macOS Intel + CI | ✅ Cross-compiles x86_64 on Apple Silicon; combined Win+Mac workflow |
| Grey gradient app icon | ✅ Neutral grey gradient replacing upstream's near-black |
| Session runtime stability | ✅ Synchronous dispose on tab close (no zombie `pi` processes); file tree refresh on run-end + focus |
| macOS model picker fix | ✅ Popover clipping fixed (WebKit stacking context + z-index) |
| Extension UI dialog fix | ✅ Invisible permission-gate confirm/select dialogs now rendered with proper CSS |
| 24 UI polish fixes | ✅ Legible text sizes, focus rings, working indicator, model-picker contrast, sidebar visibility |
